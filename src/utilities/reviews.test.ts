...
describe('community review browser utilities',()=>{
  ...
  it('creates a safe GitHub URL with the exact marker',()=>{const url=new URL(createReviewIssueUrl(reviewPayload(valid)!));expect(url.origin+url.pathname).toBe('https://github.com/GatheredApp/ForTwoPeople/issues/new');expect(url.searchParams.get('body')).toContain('<!-- REVIEW_PUBLIC_SUBMISSION_V1\n');expect(url.searchParams.get('body')).toContain('REVIEW_PUBLIC_SUBMISSION_END -->')})
