...
const valid = { ...emptyReviewDraft, name:'Test Buffet',address:'1 Main St',city:'A',state:'IN',latitude:39.1,longitude:-86.1,yelpUrl:'https://www.yelp.com/biz/test-buffet',displayName:'Dom',rating:4,review:'A sufficiently detailed review.' }
describe('community review browser utilities',()=>{
  ...
  it('creates a safe GitHub URL with the exact marker and explains community map locations',()=>{const url=new URL(createReviewIssueUrl(reviewPayload(valid)!));expect(url.origin+url.pathname).toBe('https://github.com/GatheredApp/ForTwoPeople/issues/new');expect(url.searchParams.get('body')).toContain('<!-- REVIEW_PUBLIC_SUBMISSION_V1\n');expect(url.searchParams.get('body')).toContain('REVIEW_PUBLIC_SUBMISSION_END -->');expect(url.searchParams.get('body')).toContain('community-reviewed location');expect(url.searchParams.get('body')).toContain('**Coordinates:** 39.1, -86.1');expect(url.searchParams.get('body')).toContain('**Yelp:**')})
