...
export interface ReviewDraft { buffetId: string; name: string; address: string; city: string; state: string; postalCode: string; latitude: number | ''; longitude: number | ''; yelpUrl: string; youtubeUrl: string; displayName: string; rating: number; review: string; visitDate: string; wouldReturn: '' | 'yes' | 'no' | 'unsure' }
export const emptyReviewDraft: ReviewDraft = { buffetId: '', name: '', address: '', city: '', state: '', postalCode: '', latitude:'', longitude:'', yelpUrl:'', youtubeUrl:'', displayName: '', rating: 0, review: '', visitDate: '', wouldReturn: '' }
...
export function validateReviewDraft(d: ReviewDraft) {
  ...
  if (d.youtubeUrl.trim() && !extractYouTubeId(d.youtubeUrl.trim())) errors.youtubeUrl = 'Enter a valid YouTube watch, Shorts, embed, or youtu.be URL.'
