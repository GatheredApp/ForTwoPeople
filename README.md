...
## Owner: Add a Facebook Group Review

Facebook Group Reviews are curated links to review-like posts; no post text, rating, reviewer, media, or Facebook API data is copied. Open [`?admin=facebook`](https://gatheredapp.github.io/ForTwoPeople/?admin=facebook), enter the buffet address, use **Find Coordinates** for a new location (or enter them manually), paste the HTTPS Facebook post URL, and submit the pre-filled GitHub issue while signed in as `GatheredApp`. The owner-only workflow validates its event snapshot and ingests the record automatically.

Map sources follow **Mullet Review > Community Review > Facebook Group Review** priority, so one restaurant has one marker. Facebook-only locations are dark blue; their posts dynamically move under a community or Mullet location when a higher-priority source is added. Embeds load only on request. Facebook controls whether public/group posts can be embedded, so a direct **View Facebook Post** link always remains available.
